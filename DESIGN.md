...
- `field_push` — write to a shared Field (logbook append, value ops, text template, toggle Fixed/Switch); skips when the computed value is unchanged. Logbook **Value from event** (and data **Object from event**) resolves a dotted path, a safe maths expression (`+ - * / %`, `abs`, `round`, `min`, `max`, `trunc`, `sum`, `avg`), or a JSON shape into a typed value. In shapes, quoted string leaves are literal; a leaf that is exactly one `{{ path }}` / `{{ expr }}` resolves typed; mixed text with templates is string interpolation. Elsewhere, **Templates** (`{{ }}`) are string interpolation only (actions and text fields). Pipeline actions and rule conditions may read any Field via `fields.<slug>.<path>`.
